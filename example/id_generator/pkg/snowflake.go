...
func init() {
	var serverID int64 = 1          // "unique server ID")
	var epoch int64 = 1580601600000 // "epoch time for base timeline")
	var nodeBits uint8 = 8          // "the number of bits to use for Node")
	var err error
	SnowFlake, err = newNode(serverID, epoch, nodeBits, 22-uint8(nodeBits))
	if err != nil {
		panic(err)
	}
}
