...
func parseKeyAndOptions(tagName string, field reflect.StructField) (string, *fieldOptions, error) {
	...
	cacheLock.RUnlock()
	if ok {
		return stringz.TakeOne(cache.key, field.Name), cache.options, cache.err
	}
	...
	return stringz.TakeOne(key, field.Name), options, err
}
